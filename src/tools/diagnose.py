...
if __name__ == "__main__":
    props = read_base_properties(filename="/workspace/config/qubex/64Qv1/config/props.yaml")["64Qv1"]
    info_type = (
        "qubit_frequency",
        "qubit_anharmonicity",
        "t1",
    ...
    if "x180_gate_fidelity" in info_type:
        ...
        fig = graph.create_lattice_figure(
            ...
            values=list(values.values()),
            texts=[f"{qubit}<br>{value:.2%}" if not math.isnan(value) else "N/A" for qubit, value in values.items()],
            hovertexts=[f"{qubit}: {value:.2%}" if not math.isnan(value) else "N/A" for qubit, value in values.items()],
        )
