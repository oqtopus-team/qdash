...
- ExecutionManager, TaskManager, Backend initialization
...
class SessionManager:
    ...
    Example:
        ...
        session.execution_manager.save()
        session.task_manager.get_task(...)
    ...
    def execution_manager(self) -> ExecutionManager:
        """Get the ExecutionManager instance."""
    ...
    def initialize(self) -> None:
        ...
        3. Initializes ExecutionManager
        4. Initializes TaskManager
    ...
    def _create_backend(self) -> BaseBackend:
        ...
        # task_manager must be initialized before calling this method
        assert self._task_manager is not None, "TaskManager must be initialized first"

        # Build note_path using task_manager.id
        note_path = f"{config.calib_data_path}/calib_note/{self._task_manager.id}.json"
        ...
        if backend.name == "qubex":
            backend.save_note(
                ...
                calib_dir=config.calib_data_path,
                execution_id=config.execution_id,
                task_manager_id=self._task_manager.id,
                project_id=config.project_id,
            )
    ...
    def complete(
        ...
        self._execution_manager = self.execution_manager.reload().complete_execution()
    ...
    def fail(self) -> None:
        ...
        if self._execution_manager is not None:
            self._execution_manager = self._execution_manager.reload().fail_execution()
    ...
    def _export_note_to_file(self, logger: Any) -> None:
        ...
        try:
            ...
            if latest_doc:
                note_path = Path(f"{config.calib_data_path}/calib_note/{self.task_manager.id}.json")
    ...
    def execute_task(
        ...
        # Merge results back to main task_manager
