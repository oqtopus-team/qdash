...
class FrequencyDirectionalityFilter(CRPairFilter):
    ...
    Uses either design-based inference or measured frequencies to determine
    valid CR gate directions (control frequency < target frequency).
    ...
    Example:
        ...
        # Use measured frequencies (requires calibration data)
        filter = FrequencyDirectionalityFilter(use_design_based=False)
        filtered = filter.filter(["0-1", "1-0"], context)
        ```
    """

    def __init__(self, use_design_based: bool = False):
    ...
    def filter(self, pairs: list[str], context: FilterContext) -> list[str]:
        ...
        if self.use_design_based or len(context.qubit_frequency) == 0:
        ...
        self._filtered_count = len(filtered)
        logger.info(
            f"FrequencyDirectionalityFilter ({self._method_used}): {self._total_count} → {self._filtered_count} pairs"
        )
        return filtered
    ...
    def __repr__(self) -> str:
        ...
        return f"FrequencyDirectionalityFilter(method={method})"
