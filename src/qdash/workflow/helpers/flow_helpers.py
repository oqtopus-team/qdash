...
def calibrate_qubits_task_first(
    qids: list[str],
    tasks: list[str],
    task_details: dict[str, Any] | None = None,
) -> dict[str, dict[str, Any]]:
    """Execute calibration tasks in task-first order (sequential execution).

    This function executes tasks sequentially. For each task, it processes
    all qubits one by one before moving to the next task.

    Execution pattern (sequential):
    - Task1 on Q0 → Task1 on Q1 → Task1 on Q2
    - Task2 on Q0 → Task2 on Q1 → Task2 on Q2
    - Task3 on Q0 → Task3 on Q1 → Task3 on Q2
    ...
    Note:
        This is SEQUENTIAL execution, not parallel. For true parallel execution,
        use the existing dispatch_cal_flow with ParallelNode schedule.

    Example:
        ```python
        results = calibrate_qubits_task_first(
            qids=["0", "1", "2"],
            tasks=["CheckFreq", "CheckRabi", "CheckT1"]
        )
        print(results["0"]["qubit_frequency"])
        ```

    """
    session = get_session()
    results: dict[str, dict[str, Any]] = {qid: {} for qid in qids}

    if task_details is None:
        task_details = {}

    for task_name in tasks:
        # Execute task on all qubits sequentially
        for qid in qids:
...
def calibrate_qubits_qubit_first(
    ...
    """Execute calibration tasks in qubit-first order (sequential execution).

    This function executes tasks sequentially. For each qubit, it completes
    all tasks before moving to the next qubit.

    Execution pattern (sequential):
    - Q0: Task1 → Task2 → Task3
    - Q1: Task1 → Task2 → Task3
    ...
    Note:
        This is SEQUENTIAL execution, not parallel. Use this when tasks
        have dependencies (e.g., CheckRabi depends on CheckFreq results).
        For true parallel execution, use dispatch_cal_flow with ParallelNode.

    Example:
        ```python
        results = calibrate_qubits_qubit_first(
            qids=["0", "1", "2"],
            tasks=["CheckFreq", "CheckRabi", "CheckT1"]
        )
        ```
...
def adaptive_calibrate(
    qid: str,
    measure_func: Callable[[str, int], dict[str, Any]],
    update_func: Callable[[str, dict[str, Any]], None],
    converge_func: Callable[[list[dict[str, Any]]], bool],
    max_iterations: int = 10,
) -> dict[str, Any]:
    """Execute adaptive calibration with user-defined convergence logic.

    This function enables closed-loop calibration where measurement results
    inform parameter updates until convergence is achieved.

    Args:
        qid: Qubit ID to calibrate
        measure_func: Function that performs measurement and returns results.
            Signature: (qid: str, iteration: int) -> dict
        update_func: Function that updates parameters based on results.
            Signature: (qid: str, result: dict) -> None
        converge_func: Function that checks if calibration has converged.
            Signature: (history: list[dict]) -> bool
    ...
    Returns:
        ...
        - final_result: Result from the last iteration
    ...
    Example:
        ...
        def converged(history):
            if len(history) < 2:
                return False
            diff = abs(history[-1].get("qubit_frequency", 0) -
                      history[-2].get("qubit_frequency", 0))
            return diff < 0.01

        result = adaptive_calibrate("0", measure, update, converged)
    ...
    for iteration in range(max_iterations):
        ...
        # Update parameters if present in result
