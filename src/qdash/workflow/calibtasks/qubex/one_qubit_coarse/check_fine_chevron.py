...
class CheckFineChevron(QubexTask):
    ...
    def postprocess(
        ...
        if qubit_freq is not None and qubit_freq < 2.5:
            error_msg = f"Qubit frequency too low for qid={qid}: {qubit_freq:.6f} GHz < 2.5 GHz"
    ...
    def run(self, backend: QubexBackend, qid: str) -> RunResult:
        ...
        if readout_frequency.value is None or qubit_frequency.value is None:
            raise ValueError("Chevron frequency inputs were not resolved during preprocessing")

        readout_amp = self._get_readout_amplitude_value()

        control_amplitude = self.input_parameters["control_amplitude"].value
        if control_amplitude is None:
            raise ValueError("control_amplitude input parameter is required")
