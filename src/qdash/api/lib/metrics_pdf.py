...
class MetricsPDFGenerator:
    ...
    def generate_pdf(self) -> io.BytesIO:
        ...
        # CDF pages for qubit metric groups (skip empty groups)
        for cdf_group in self.config.cdf_groups.qubit:
            if self._has_cdf_data(cdf_group, "qubit"):
                self._draw_cdf_page(
                    c,
                    cdf_group=cdf_group,
                    metric_type="qubit",
        ...
        # CDF pages for coupling metric groups (skip empty groups)
        for cdf_group in self.config.cdf_groups.coupling:
            if self._has_cdf_data(cdf_group, "coupling"):
                self._draw_cdf_page(
                    c,
                    cdf_group=cdf_group,
                    metric_type="coupling",
    ...
    def _draw_cover_page(self, c: canvas.Canvas) -> None:
        ...
        c.roundRect(
            box_left + 3, box_top - box_height - 3, box_width, box_height, 10, fill=1, stroke=0
        )
        ...
        c.setFont("Helvetica", 10)
        c.drawCentredString(
            self.width / 2, 50, "Generated by QDash - Quantum Device Calibration Dashboard"
        )

    def _draw_summary_page(self, c: canvas.Canvas) -> None:
        """Draw a summary page with all metrics statistics in a table."""
        # Header bar
        header_height = 50
        c.setFillColor(colors.Color(0.2, 0.3, 0.5))
        ...
        c.drawString(50, self.height - 33, "Metrics Summary")
        ...
        col_widths = [0.28, 0.12, 0.15, 0.15, 0.15, 0.15]  # Title, Unit, Coverage, Median, Min, Max
        col_positions = [table_left]
        for w in col_widths[:-1]:
            col_positions.append(col_positions[-1] + table_width * w)

        # Table headers
        headers = ["Metric", "Unit", "Coverage", "Median", "Min", "Max"]

        # Start position
        y_pos = self.height - header_height - 30

        # Draw table header row
        c.setFillColor(colors.Color(0.2, 0.3, 0.5))
        c.rect(
            table_left, y_pos - header_row_height, table_width, header_row_height, fill=1, stroke=0
        )
        ...
        for i, header in enumerate(headers):
            if i == 0:
                c.drawString(col_positions[i] + 8, y_pos - 20, header)
            else:
                # Center align other columns
                text_width = c.stringWidth(header, "Helvetica-Bold", 10)
                col_width = table_width * col_widths[i]
                c.drawString(col_positions[i] + (col_width - text_width) / 2, y_pos - 20, header)
        ...
        for idx, metric in enumerate(all_metrics):
            ...
            c.setLineWidth(0.5)
            c.line(table_left, y_pos - row_height, table_left + table_width, y_pos - row_height)
            ...
            c.drawString(col_positions[0] + 8, y_pos - 16, title_text)

            # Unit (center aligned)
            unit_text = metric["unit"]
            col_width = table_width * col_widths[1]
            text_width = c.stringWidth(unit_text, "Helvetica", 9)
            c.drawString(col_positions[1] + (col_width - text_width) / 2, y_pos - 16, unit_text)

            # Coverage (center aligned)
            coverage_text = (
                f"{stats['coverage']:.1f}% ({stats['coverage_count']}/{stats['total_count']})"
            )
            col_width = table_width * col_widths[2]
            text_width = c.stringWidth(coverage_text, "Helvetica", 9)
            c.drawString(col_positions[2] + (col_width - text_width) / 2, y_pos - 16, coverage_text)

            # Median (center aligned)
            median_text = self._format_stat_value(stats["median"])
            col_width = table_width * col_widths[3]
            text_width = c.stringWidth(median_text, "Helvetica", 9)
            c.drawString(col_positions[3] + (col_width - text_width) / 2, y_pos - 16, median_text)

            # Min (center aligned)
            min_text = self._format_stat_value(stats["minimum"])
            col_width = table_width * col_widths[4]
            text_width = c.stringWidth(min_text, "Helvetica", 9)
            c.drawString(col_positions[4] + (col_width - text_width) / 2, y_pos - 16, min_text)

            # Max (center aligned)
            max_text = self._format_stat_value(stats["maximum"])
            col_width = table_width * col_widths[5]
            text_width = c.stringWidth(max_text, "Helvetica", 9)
            c.drawString(col_positions[5] + (col_width - text_width) / 2, y_pos - 16, max_text)

            y_pos -= row_height

        # Table outer border
        table_height = header_row_height + row_height * len(all_metrics)
        c.setStrokeColor(colors.Color(0.7, 0.7, 0.7))
        c.setLineWidth(1)
        c.rect(
            table_left,
            self.height - header_height - 30 - table_height,
            table_width,
            table_height,
            fill=0,
            stroke=1,
        ...
        c.drawRightString(self.width - 50, 25, "Summary")
    ...
    def _draw_metric_page(
        ...
        c.roundRect(
            self.width - unit_width - 50, self.height - 40, unit_width, 22, 5, fill=1, stroke=0
        )
        c.setFillColor(colors.white)
        c.setFont("Helvetica", 10)
    ...
    def _draw_statistics_box(
        ...
        stat_items = [
            (
                "Coverage",
                f"{stats['coverage']:.1f}%",
                f"({stats['coverage_count']}/{stats['total_count']})",
            ),
    ...
    def _has_cdf_data(
        ...
        """Check if a CDF group has any data."""
        metrics_source = (
            self.metrics_response.qubit_metrics
            if metric_type == "qubit"
            else self.metrics_response.coupling_metrics
        )
        metrics_config = (
            self.config.qubit_metrics if metric_type == "qubit" else self.config.coupling_metrics
        )

        for metric_key in cdf_group.metrics:
            metric_meta = metrics_config.get(metric_key)
            if not metric_meta:
                continue
    ...
    def _draw_cdf_page(
        ...
        """Draw a CDF chart page for a metric group."""
        # Header bar
        header_height = 50
        c.setFillColor(colors.Color(0.2, 0.3, 0.5))
        c.rect(0, self.height - header_height, self.width, header_height, fill=1, stroke=0)
        ...
        c.setFillColor(colors.white)
        c.setFont("Helvetica-Bold", 18)
        c.drawString(50, self.height - 33, f"CDF - {cdf_group.title}")

        # Unit badge
        c.setFillColor(colors.Color(0.3, 0.4, 0.6))
        unit_text = f"Unit: {cdf_group.unit}"
        unit_width = c.stringWidth(unit_text, "Helvetica", 10) + 16
        c.roundRect(
            self.width - unit_width - 50, self.height - 40, unit_width, 22, 5, fill=1, stroke=0
        )
        ...
        # Generate CDF chart
        fig = self._create_cdf_chart(cdf_group, metric_type)
        ...
        if fig:
            ...
            max_img_height = self.height - 130
            if img_height > max_img_height:
                img_height = max_img_height
                img_width = img_height / aspect
            ...
            img_x = (self.width - img_width) / 2
            img_y = self.height - header_height - img_height - 30
            ...
            c.roundRect(
                ...
                img_width + border_padding * 2,
                img_height + border_padding * 2,
                5,
                fill=0,
                stroke=1,
            )
    ...
    def _create_cdf_chart(
        ...
        """Create a CDF chart for a metric group using Plotly."""
        ...
        for idx, metric_key in enumerate(cdf_group.metrics):
            # Get metric config and data
            metric_meta = metrics_config.get(metric_key)
            if not metric_meta:
                continue
            ...
            for i, value in enumerate(values):
                cdf_x.append(value)
                cdf_y.append(((i + 1) / n) * 100)

            cdf_x.append(max_val + extension)  # End flat at 100%
            cdf_y.append(100)
