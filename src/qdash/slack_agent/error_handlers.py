...
class MetricsCollector:
    ...
    def get_health_status(self) -> Dict[str, Any]:
        ...
        success_rate = (
            self.metrics["successful_responses"] / max(self.metrics["user_interactions"], 1) * 100
        )
        
        avg_response_time = (
            sum(self.metrics["response_times"]) / max(len(self.metrics["response_times"]), 1)
        )
    ...
    async def wrapper(*args, **kwargs):
        ...
        try:
            ...
            if "database" in str(e).lower() or "mongo" in str(e).lower():
                return f"⚠️ データベース接続エラーが発生しました。管理者に連絡してください。"
            elif "timeout" in str(e).lower():
                return f"⏱️ 処理がタイムアウトしました。しばらく待ってから再試行してください。"
            elif "permission" in str(e).lower() or "auth" in str(e).lower():
                return f"🔒 認証エラーが発生しました。権限を確認してください。"
            else:
