...
class ConfigLoader:
    ...
    LOCAL_CONFIG_DIR: Path = Path(__file__).parent.parent.parent.parent / "config"
    ...
    def load_backend(cls) -> dict[str, Any]:
        ...
        -------
        dict
            Backend config from backend.yaml merged with backend.local.yaml.
            Contains:
            - default_backend: str
            - backends: dict with tasks per backend
            - categories: dict grouping tasks by category
