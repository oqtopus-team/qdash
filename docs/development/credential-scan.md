...
| `.devcontainer/requirements.txt` | path | `uv export --no-hashes` output (~900 lines of pinned dependencies). The wide surface of package URLs and version strings produces incidental pattern matches; regenerated by `uv export`. |
