...
Both clients use the Orval version locked in this package. Install its dependencies with
`bun install --frozen-lockfile` before running `task generate` from the repository root.
`bun run generate:ui` regenerates the UI client; `bun run generate` regenerates this client.
