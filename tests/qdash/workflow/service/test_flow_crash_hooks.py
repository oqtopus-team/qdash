...
def test_on_flow_crashed_keep_lock_closes_execution_without_releasing_lock(init_db) -> None:
    """on_flow_crashed_keep_lock closes the execution but leaves the project lock alone."""
    _make_execution(status="running")
    _make_task(status="running", task_id="task-running")
    _make_task(status="completed", task_id="task-completed")
...
def test_on_flow_cancellation_keep_lock_closes_execution_without_releasing_lock(init_db) -> None:
    """on_flow_cancellation_keep_lock closes the execution but leaves the project lock alone."""
    _make_execution(status="running")
    _make_task(status="running", task_id="task-running")
    _make_task(status="completed", task_id="task-completed")
    ExecutionLockDocument(project_id=PROJECT_ID, locked=True, execution_id="exec-1").save()

    on_flow_cancellation_keep_lock(None, _fake_flow_run(), None)
...
def test_on_flow_failure_keep_lock_closes_execution_without_releasing_lock(init_db) -> None:
    """on_flow_failure_keep_lock closes the execution but leaves the project lock alone."""
    _make_execution(status="running")
    _make_task(status="running", task_id="task-running")
    _make_task(status="completed", task_id="task-completed")
    ExecutionLockDocument(project_id=PROJECT_ID, locked=True, execution_id="exec-1").save()

    on_flow_failure_keep_lock(None, _fake_flow_run(), None)
