...
class TestCallLlmExtract:
    ...
    async def test_returns_empty_when_copilot_disabled(self):
        ...
        with patch("qdash.common.copilot.settings.load_copilot_config", return_value=config):
    ...
    async def test_parses_valid_json_response(self, mock_config):
        ...
        with (
            patch("qdash.common.copilot.settings.load_copilot_config", return_value=mock_config),
            patch("qdash.common.copilot.llm_agent._build_client", return_value=mock_client),
        ):
            result = await IssueKnowledgeService._call_llm_extract(
