...
/**
 * Animated counter that only animates when element is in viewport
 */
export function AnimatedCounterOnView({
  value,
  duration = 1000,
  ...props
}: AnimatedCounterProps) {
  ...
  return (
    <span ref={ref}>
      {isVisible ? (
        <AnimatedCounter value={value} duration={duration} {...props} />
      ) : (
