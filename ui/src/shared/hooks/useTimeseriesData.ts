...
import { TimeSeriesDataPoint, TimeRangeState, ParameterKey, TagKey } from '../types/analysis';
    ...
    try {
      if (qubitId) {
        ...
        Object.entries(timeseriesResponse.data.data).forEach(([qid, dataPoints]) => {
          if (Array.isArray(dataPoints)) {
            qidData[qid] = {
              x: dataPoints.map((point: OutputParameterModel) => point.calibrated_at || ''),
              ...
              error: dataPoints.map((point: OutputParameterModel) => point.error || 0),
            };
          }
        });
