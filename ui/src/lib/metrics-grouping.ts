...
/** Group used for metrics without an explicit category. Always rendered last. */
const FALLBACK_CATEGORY = "Other";

/**
 * Group metric options by their `category` for use in react-select.
 *
 * Categories are emitted in the order they first appear in the provided
 * metrics list, so the logical ordering defined in `config/domain/metrics.yaml`
 * is preserved. Metrics without a category fall back to an "Other" group that is
 * always placed last.
 *
...
export function groupMetricsByCategory(metrics: MetricConfig[]): GroupBase<MetricOption>[] {
  ...
  for (const metric of metrics) {
    const category = metric.category?.trim() || FALLBACK_CATEGORY;
