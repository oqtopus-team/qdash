...
import { useListChips, useFetchLatestQubitTaskGroupedByChip, useFetchHistoricalQubitTaskGroupedByChip } from "@/client/chip/chip";
...
const PARAMETER_CONFIG: Record<string, { 
  ...
  gate_fidelity: { label: "Gate Fidelity (Clifford RB)", unit: "fidelity", displayUnit: "fidelity" },
  x90_fidelity: { label: "X90 Gate Fidelity", unit: "fidelity", displayUnit: "fidelity" },
  x180_fidelity: { label: "X180 Gate Fidelity", unit: "fidelity", displayUnit: "fidelity" },
  zx90_fidelity: { label: "ZX90 Gate Fidelity (2Q)", unit: "fidelity", displayUnit: "fidelity" },
  readout_fidelity: { label: "Readout Fidelity", unit: "fidelity", displayUnit: "fidelity" },
};
...
function calculateCorrelationMatrix(
  ...
  for (let i = 0; i < parameters.length; i++) {
    for (let j = i + 1; j < parameters.length; j++) {
      ...
      if (validPoints.length >= 3) { // Minimum 3 points for meaningful correlation
...
function generateHeatmapData(
  ...
  parameters: string[]
): { z: number[][]; x: string[]; y: string[]; text: string[][]; hovertext: string[][] } {
  const n = parameters.length;
  const correlationMatrix: number[][] = Array(n).fill(null).map(() => Array(n).fill(0));
  const textMatrix: string[][] = Array(n).fill(null).map(() => Array(n).fill(''));
  const hovertextMatrix: string[][] = Array(n).fill(null).map(() => Array(n).fill(''));
  
  // Fill diagonal with 1.0 (self-correlation)
  for (let i = 0; i < n; i++) {
    correlationMatrix[i][i] = 1.0;
    textMatrix[i][i] = '1.000';
    hovertextMatrix[i][i] = `${PARAMETER_CONFIG[parameters[i]].label}<br>Self-correlation: 1.000`;
  }
...
export function CorrelationView() {
  ...
  const [selectedParameters, setSelectedParameters] = useState<string[]>(["t1", "t2_echo", "t2_star", "gate_fidelity"]);
  ...
  const [scatterParameterY, setScatterParameterY] = useState<string>("gate_fidelity");
  ...
  const t1LatestQuery = useFetchLatestQubitTaskGroupedByChip(
    ...
    {
      query: {
        enabled: Boolean(selectedChip && TASK_CONFIG.t1?.name && selectedDate === 'latest'),
  ...
  const t2EchoLatestQuery = useFetchLatestQubitTaskGroupedByChip(
    ...
    {
      query: {
        enabled: Boolean(selectedChip && TASK_CONFIG.t2_echo?.name && selectedDate === 'latest'),
        refetchInterval: selectedDate === 'latest' ? 30000 : undefined,
        staleTime: 25000,
      },
    }
  ...
  const t2StarLatestQuery = useFetchLatestQubitTaskGroupedByChip(
    ...
    {
      query: {
        enabled: Boolean(selectedChip && TASK_CONFIG.t2_star?.name && selectedDate === 'latest'),
        refetchInterval: selectedDate === 'latest' ? 30000 : undefined,
        staleTime: 25000,
      },
    }
  ...
  const gateFidelityLatestQuery = useFetchLatestQubitTaskGroupedByChip(
    ...
    {
      query: {
        enabled: Boolean(selectedChip && TASK_CONFIG.gate_fidelity?.name && selectedDate === 'latest'),
        refetchInterval: selectedDate === 'latest' ? 30000 : undefined,
        staleTime: 25000,
      },
    }
  ...
  const x90FidelityLatestQuery = useFetchLatestQubitTaskGroupedByChip(
    ...
    {
      query: {
        enabled: Boolean(selectedChip && TASK_CONFIG.x90_fidelity?.name && selectedDate === 'latest'),
        refetchInterval: selectedDate === 'latest' ? 30000 : undefined,
        staleTime: 25000,
      },
    }
  ...
  const x180FidelityLatestQuery = useFetchLatestQubitTaskGroupedByChip(
    ...
    {
      query: {
        enabled: Boolean(selectedChip && TASK_CONFIG.x180_fidelity?.name && selectedDate === 'latest'),
        refetchInterval: selectedDate === 'latest' ? 30000 : undefined,
        staleTime: 25000,
      },
    }
  ...
  const readoutFidelityLatestQuery = useFetchLatestQubitTaskGroupedByChip(
    ...
    {
      query: {
        enabled: Boolean(selectedChip && TASK_CONFIG.readout_fidelity?.name && selectedDate === 'latest'),
        refetchInterval: selectedDate === 'latest' ? 30000 : undefined,
        staleTime: 25000,
      },
    }
  ...
  const readoutFidelityHistoricalQuery = useFetchHistoricalQubitTaskGroupedByChip(
    selectedChip,
    TASK_CONFIG.readout_fidelity?.name || "",
    selectedDate,
    {
  ...
  const parameterDataQueries = useMemo(() => {
    ...
    const allParameterQueries = [
      {
        ...
        data: selectedDate === 'latest' ? t1LatestQuery.data : t1HistoricalQuery.data,
        isLoading: selectedDate === 'latest' ? t1LatestQuery.isLoading : t1HistoricalQuery.isLoading,
        error: selectedDate === 'latest' ? t1LatestQuery.error : t1HistoricalQuery.error,
      },
      {
        parameter: 't2_echo',
        taskName: TASK_CONFIG.t2_echo?.name,
        data: selectedDate === 'latest' ? t2EchoLatestQuery.data : t2EchoHistoricalQuery.data,
        isLoading: selectedDate === 'latest' ? t2EchoLatestQuery.isLoading : t2EchoHistoricalQuery.isLoading,
        error: selectedDate === 'latest' ? t2EchoLatestQuery.error : t2EchoHistoricalQuery.error,
      },
      {
        parameter: 't2_star',
        taskName: TASK_CONFIG.t2_star?.name,
        data: selectedDate === 'latest' ? t2StarLatestQuery.data : t2StarHistoricalQuery.data,
        isLoading: selectedDate === 'latest' ? t2StarLatestQuery.isLoading : t2StarHistoricalQuery.isLoading,
        error: selectedDate === 'latest' ? t2StarLatestQuery.error : t2StarHistoricalQuery.error,
      },
      {
        parameter: 'gate_fidelity',
        taskName: TASK_CONFIG.gate_fidelity?.name,
        data: selectedDate === 'latest' ? gateFidelityLatestQuery.data : gateFidelityHistoricalQuery.data,
        isLoading: selectedDate === 'latest' ? gateFidelityLatestQuery.isLoading : gateFidelityHistoricalQuery.isLoading,
        error: selectedDate === 'latest' ? gateFidelityLatestQuery.error : gateFidelityHistoricalQuery.error,
      },
      {
        parameter: 'x90_fidelity',
        taskName: TASK_CONFIG.x90_fidelity?.name,
        data: selectedDate === 'latest' ? x90FidelityLatestQuery.data : x90FidelityHistoricalQuery.data,
        isLoading: selectedDate === 'latest' ? x90FidelityLatestQuery.isLoading : x90FidelityHistoricalQuery.isLoading,
        error: selectedDate === 'latest' ? x90FidelityLatestQuery.error : x90FidelityHistoricalQuery.error,
      },
      {
        parameter: 'x180_fidelity',
        taskName: TASK_CONFIG.x180_fidelity?.name,
        data: selectedDate === 'latest' ? x180FidelityLatestQuery.data : x180FidelityHistoricalQuery.data,
        isLoading: selectedDate === 'latest' ? x180FidelityLatestQuery.isLoading : x180FidelityHistoricalQuery.isLoading,
        error: selectedDate === 'latest' ? x180FidelityLatestQuery.error : x180FidelityHistoricalQuery.error,
      },
      {
        parameter: 'readout_fidelity',
        taskName: TASK_CONFIG.readout_fidelity?.name,
        data: selectedDate === 'latest' ? readoutFidelityLatestQuery.data : readoutFidelityHistoricalQuery.data,
        isLoading: selectedDate === 'latest' ? readoutFidelityLatestQuery.isLoading : readoutFidelityHistoricalQuery.isLoading,
        error: selectedDate === 'latest' ? readoutFidelityLatestQuery.error : readoutFidelityHistoricalQuery.error,
      },
    ];
    ...
    selectedDate,
    // T1 queries
    t1LatestQuery.data, t1LatestQuery.isLoading, t1LatestQuery.error,
    t1HistoricalQuery.data, t1HistoricalQuery.isLoading, t1HistoricalQuery.error,
    // T2 Echo queries
    t2EchoLatestQuery.data, t2EchoLatestQuery.isLoading, t2EchoLatestQuery.error,
    t2EchoHistoricalQuery.data, t2EchoHistoricalQuery.isLoading, t2EchoHistoricalQuery.error,
    // T2 Star queries
    t2StarLatestQuery.data, t2StarLatestQuery.isLoading, t2StarLatestQuery.error,
    t2StarHistoricalQuery.data, t2StarHistoricalQuery.isLoading, t2StarHistoricalQuery.error,
    // Gate Fidelity queries
    gateFidelityLatestQuery.data, gateFidelityLatestQuery.isLoading, gateFidelityLatestQuery.error,
    gateFidelityHistoricalQuery.data, gateFidelityHistoricalQuery.isLoading, gateFidelityHistoricalQuery.error,
    // X90 Fidelity queries
    x90FidelityLatestQuery.data, x90FidelityLatestQuery.isLoading, x90FidelityLatestQuery.error,
    x90FidelityHistoricalQuery.data, x90FidelityHistoricalQuery.isLoading, x90FidelityHistoricalQuery.error,
    // X180 Fidelity queries
    x180FidelityLatestQuery.data, x180FidelityLatestQuery.isLoading, x180FidelityLatestQuery.error,
    x180FidelityHistoricalQuery.data, x180FidelityHistoricalQuery.isLoading, x180FidelityHistoricalQuery.error,
    // Readout Fidelity queries
    readoutFidelityLatestQuery.data, readoutFidelityLatestQuery.isLoading, readoutFidelityLatestQuery.error,
    readoutFidelityHistoricalQuery.data, readoutFidelityHistoricalQuery.isLoading, readoutFidelityHistoricalQuery.error,
  ]);
  ...
  const { correlationResults, isLoading, error } = useMemo(() => {
    ...
    console.log('CorrelationView Debug:', {
      ...
      parameterDataQueries: parameterDataQueries?.map(q => ({
        ...
        sampleData: q.data?.data?.result ? Object.entries(q.data.data.result).slice(0, 1) : []
      }))
    });

    // Check if any queries are loading
    const anyLoading = parameterDataQueries?.some(query => query.isLoading) || false;
    ...
    Array.from(qubitIds).forEach(qid => {
      ...
      parameterDataQueries.forEach(query => {
        if (query.data?.data?.result?.[qid]?.output_parameters) {
          ...
          const paramValue = query.data.data.result[qid].output_parameters[outputParamName];
          
          console.log(`Debug ${qid} ${query.parameter}:`, {
            outputParamName,
            rawValue: paramValue,
            valueType: typeof paramValue,
            rawValueKeys: typeof paramValue === 'object' && paramValue !== null ? Object.keys(paramValue) : null,
            rawValueContent: typeof paramValue === 'object' && paramValue !== null ? paramValue : null
          });
          ...
          if (paramValue !== null && paramValue !== undefined) {
            ...
            if (typeof paramValue === 'number') {
              ...
              if ('value' in paramValue && typeof paramValue.value === 'number') {
                ...
                console.warn(`Unknown object structure for ${query.parameter}:`, paramValue);
                return;
              }
            } else {
              console.warn(`Cannot process value type for ${query.parameter}:`, typeof paramValue);
              return;
            }
    ...
    const correlationResults = calculateCorrelationMatrix(dataPoints, selectedParameters);
  ...
  const heatmapPlotData = useMemo(() => {
    if (selectedParameters.length < 2 || isLoading || correlationResults.length === 0) {
      return [];
    }

    const heatmapData = generateHeatmapData(correlationResults, selectedParameters);
  ...
  const scatterPlotData = useMemo(() => {
    if (viewMode !== "scatter" || !scatterParameterX || !scatterParameterY || scatterParameterX === scatterParameterY) {
      return [];
    }

    // Find the queries for the selected parameters (now always available)
    const paramXQuery = parameterDataQueries?.find(q => q.parameter === scatterParameterX);
    const paramYQuery = parameterDataQueries?.find(q => q.parameter === scatterParameterY);

    if (!paramXQuery?.data?.data?.result || !paramYQuery?.data?.data?.result) {
      return [];
    }

    const scatterPoints: Array<{x: number; y: number; text: string; qid: string}> = [];
  ...
  }, [viewMode, scatterParameterX, scatterParameterY, parameterDataQueries, extractNumericValue]);
  ...
  const handleExportCSV = () => {
    ...
    const headers = ['Parameter_A', 'Parameter_B', 'Correlation', 'Sample_Size', 'Abs_Correlation', 'Chip', 'Date'];
    ...
    <div className="space-y-6">
        ...
        <div className="card-body">
          ...
          <div className="flex flex-wrap items-end gap-4">
            ...
            <div className="form-control min-w-48">
              <div className="h-8"></div> {/* Spacer to align with other form controls */}
              <div className="h-10">
                <ChipSelector
            ...
            {viewMode === "heatmap" ? (
              ...
              <>
                ...
                <div className="form-control min-w-48">
                  <div className="flex justify-between items-center h-8">
                    <span className="label-text font-semibold">X-axis Parameter</span>
                  </div>
                  <div className="h-10">
                    <Select<{ value: string; label: string }>
                      options={availableParameters}
                      value={availableParameters.find(option => option.value === scatterParameterX)}
                      onChange={(option) => {
                        if (option) {
                ...
                <div className="form-control min-w-48">
                  <div className="flex justify-between items-center h-8">
                    <span className="label-text font-semibold">Y-axis Parameter</span>
                  </div>
                  <div className="h-10">
                    <Select<{ value: string; label: string }>
                      options={availableParameters}
                      value={availableParameters.find(option => option.value === scatterParameterY)}
                      onChange={(option) => {
                        if (option) {
            ...
            <div className="form-control min-w-32">
              ...
              <div className="h-10">
                <button
                  ...
                  disabled={
                    viewMode === "heatmap" 
                      ? selectedParameters.length < 2 || correlationResults.length === 0
      ...
      {error && (
        <div className="alert alert-error">
          ...
          <span>Failed to load correlation data: {error.message || "An unexpected error occurred"}</span>
        </div>
      )}
      ...
      {viewMode === "heatmap" ? (
        // Correlation Heatmap
        selectedParameters.length >= 2 && !isLoading && correlationResults.length > 0 ? (
          <PlotCard
                ...
                {selectedParameters.length < 2 ? (
                  <div className="flex items-center justify-center h-96 bg-base-200 rounded-lg">
                    <div className="text-center">
                      <div className="text-lg font-semibold mb-2">Parameter Correlation Analysis</div>
                  ...
                  <div className="flex items-center justify-center h-96 bg-base-200 rounded-lg">
                    <div className="text-center">
                      <div className="text-lg font-semibold mb-2">Loading Correlation Data</div>
                      <div className="text-base-content/60 mb-4">
                        Analyzing correlations between: {selectedParameters.map(p => PARAMETER_CONFIG[p].label).join(", ")}
                      </div>
            ...
            <div className="card-body">
              <div className="min-h-96">
                {scatterParameterX === scatterParameterY ? (
                  <div className="flex items-center justify-center h-96 bg-base-200 rounded-lg">
                    <div className="text-center">
                      <div className="text-lg font-semibold mb-2">Parameter Scatter Plot</div>
                      <div className="text-base-content/60">
                  ...
                  <div className="flex items-center justify-center h-96 bg-base-200 rounded-lg">
                    <div className="text-center">
                      <div className="text-lg font-semibold mb-2">Loading Scatter Plot Data</div>
                      <div className="text-base-content/60 mb-4">
                        Analyzing: {PARAMETER_CONFIG[scatterParameterX].label} vs {PARAMETER_CONFIG[scatterParameterY].label}
                      </div>
                      <div className="loading loading-spinner loading-lg"></div>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-center h-96 bg-base-200 rounded-lg">
                    <div className="text-center">
                      <div className="text-lg font-semibold mb-2">No Scatter Plot Data</div>
                      <div className="text-base-content/60">
                        Insufficient data for scatter plot analysis.<br/>
                        Try selecting different parameters or date.
                      </div>
                    </div>
                  </div>
      ...
      {viewMode === "heatmap" && selectedParameters.length >= 2 && !isLoading && correlationResults.length > 0 && (
        <div className="card bg-base-100 shadow-md">
          <div className="card-body">
            <div className="space-y-4">
              ...
              <div className="overflow-x-auto">
                <table className="table table-zebra w-full">
                  ...
                  <tbody>
                    {correlationResults
                      .filter(result => Math.abs(result.correlation) >= correlationThreshold)
                      .sort((a, b) => Math.abs(b.correlation) - Math.abs(a.correlation))
                      .map((result, index) => {
                        const absCorr = Math.abs(result.correlation);
                        const strength = absCorr >= 0.7 ? 'Strong' : absCorr >= 0.3 ? 'Moderate' : 'Weak';
                        const strengthColor = absCorr >= 0.7 ? 'text-success' : absCorr >= 0.3 ? 'text-warning' : 'text-info';
                        
                        return (
                          <tr key={index}>
                            <td className="font-medium">{PARAMETER_CONFIG[result.parameterA].label}</td>
                            <td className="font-medium">{PARAMETER_CONFIG[result.parameterB].label}</td>
                            <td className="font-mono">
                              <span className={result.correlation >= 0 ? 'text-success' : 'text-error'}>
              ...
              {correlationResults.filter(result => Math.abs(result.correlation) >= correlationThreshold).length === 0 && (
                <div className="text-center py-8 text-base-content/60">
                  No correlations above threshold ({correlationThreshold.toFixed(2)})
                </div>
              )}
            </div>
          </div>
      ...
      {/* Statistics Section */}
      {((viewMode === "heatmap" && selectedParameters.length >= 2 && correlationResults.length > 0) ||
            ...
            {viewMode === "heatmap" ? (
              <div className="stats shadow grid-cols-2 lg:grid-cols-5">
                <div className="stat">
                  ...
                  <div className="stat-value text-sm">{selectedParameters.length}</div>
                  <div className="stat-desc">Selected for analysis</div>
                </div>
                <div className="stat">
                  <div className="stat-title">Valid Pairs</div>
                  <div className="stat-value text-sm">{correlationResults.length}</div>
                  <div className="stat-desc">With sufficient data</div>
                </div>
                <div className="stat">
                  <div className="stat-title">Above Threshold</div>
                  <div className="stat-value text-sm">
                    {correlationResults.filter(result => Math.abs(result.correlation) >= correlationThreshold).length}
                  </div>
                  <div className="stat-desc">≥ {correlationThreshold.toFixed(2)} correlation</div>
                </div>
                <div className="stat">
                  <div className="stat-title">Strongest</div>
                  <div className="stat-value text-sm">
                    {correlationResults.length > 0 ? 
                      Math.max(...correlationResults.map(r => Math.abs(r.correlation))).toFixed(3) : 
                      'N/A'
                    }
                  </div>
                ...
                <div className="stat">
                  ...
                  <div className="stat-value text-sm">
                    {correlationResults.length > 0 ? 
                      Math.round(correlationResults.reduce((sum, r) => sum + r.sampleSize, 0) / correlationResults.length) : 
                      'N/A'
                    }
                  </div>
              ...
              <div className="stats shadow grid-cols-2 lg:grid-cols-4">
                <div className="stat">
                  ...
                  <div className="stat-value text-sm">{PARAMETER_CONFIG[scatterParameterX].label}</div>
                  <div className="stat-desc">Horizontal axis</div>
                </div>
                <div className="stat">
                  <div className="stat-title">Y Parameter</div>
                  <div className="stat-value text-sm">{PARAMETER_CONFIG[scatterParameterY].label}</div>
