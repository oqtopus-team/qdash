...
export function QubitRadarChart({ qubitId, taskData, isLoading = false, error }: QubitRadarChartProps) {
    ...
    Object.entries(PARAMETER_CONFIG).forEach(([key, config]) => {
      ...
      if (data) {
        ...
        if (rawValue !== null && !isNaN(rawValue) && rawValue > 0) {
          ...
          const normalizedValue = Math.min((rawValue / config.maxValue) * 100, 100);
  ...
  const overallScore = useMemo(() => {
    ...
    return Math.round(radarData.reduce((sum, p) => sum + p.value, 0) / radarData.length);
  }, [radarData]);

  // Get performance rank based on overall score
  const getPerformanceRank = (score: number) => {
    if (score >= 90) return { rank: "S", color: "text-yellow-500", bg: "bg-yellow-100" };
    if (score >= 80) return { rank: "A", color: "text-green-500", bg: "bg-green-100" };
    if (score >= 70) return { rank: "B", color: "text-blue-500", bg: "bg-blue-100" };
    if (score >= 60) return { rank: "C", color: "text-purple-500", bg: "bg-purple-100" };
    if (score >= 50) return { rank: "D", color: "text-orange-500", bg: "bg-orange-100" };
  ...
  if (error) {
    return (
      <ErrorCard
        ...
        message={error.message || "Failed to load qubit performance data"}
        ...
        <div className="card-body">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h2 className="text-2xl font-bold">Qubit {qubitId} Performance</h2>
                <p className="text-base-content/60">Quantum bit capability analysis</p>
              </div>
            </div>
            <div className="text-center">
              <div className={`text-6xl font-bold ${performanceRank.color} mb-2`}>
                {performanceRank.rank}
              </div>
              <div className={`badge badge-lg ${performanceRank.bg} ${performanceRank.color} font-semibold`}>
                {overallScore}/100
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {radarData.map((param, index) => (
              <div key={index} className="stat bg-base-200 rounded-lg p-4">
                ...
                <div className="stat-title text-sm font-medium">{param.name}</div>
                <div className="stat-value text-lg" style={{ color: param.color }}>
                  {param.value.toFixed(1)}/100
                </div>
                <div className="stat-desc text-xs">
                  Raw: {((param.value / 100) * param.maxValue).toFixed(param.unit ? 2 : 4)}{param.unit}
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="card-body">
          ...
          <div className="space-y-3">
            {overallScore >= 90 && (
              <div className="alert alert-success">
                ...
                <span><strong>Excellent Performance!</strong> This qubit is performing at the highest level across all parameters.</span>
              </div>
            )}
            {overallScore >= 70 && overallScore < 90 && (
              <div className="alert alert-info">
                ...
                <span><strong>Good Performance.</strong> This qubit shows solid performance with room for optimization.</span>
              </div>
            )}
            {overallScore < 70 && (
              <div className="alert alert-warning">
                ...
                <span><strong>Performance Issues Detected.</strong> Consider recalibration or hardware inspection.</span>
              </div>
            )}
          </div>
          ...
          <div className="mt-4 text-sm text-base-content/70">
            ...
            <ul className="list-disc list-inside space-y-1 ml-4">
              <li>T1 Coherence: Normalized to 200µs maximum (energy relaxation time)</li>
              <li>T2 Echo/Star: Normalized to 400µs/100µs respectively (dephasing times)</li>
              <li>Gate Fidelity: Percentage of successful quantum operations</li>
